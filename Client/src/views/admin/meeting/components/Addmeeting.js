...
const AddMeeting = (props) => {
    ...
    const AddData = async () => {
        try {
            setIsLoding(true);
            // Prepare the data based on form values
            const meetingData = {
            ...
            let response = await postApi('api/meeting/add', meetingData);

            if (response?.status === 200 || response?.status === 201) {
                toast.success('Meeting added successfully!');
                setAction(prev => !prev);

                // If fetchData function is provided, call it to refresh the data
                if (fetchData) {
                    fetchData();
                }
    ...
    const fetchAllData = async () => {
        try {
            ...
            // If related is Contact, fetch contacts
            if (values.related === 'Contact') {
                // Check if contacts already exist in redux
            ...
            // If related is Lead, fetch leads
            if (values.related === 'Lead') {
                // Check if leads already exist in redux
    ...
    useEffect(() => {
        // Fetch data when component mounts or when related value changes
        fetchAllData();

        // If the component is opened from contact or lead view with an ID
        if (props.id) {
            if (props.leadContect === 'contactView') {
                // If specific contact is already selected
                setFieldValue('related', 'Contact');
                setFieldValue('attendes', [props.id]);
            } else if (props.leadContect === 'leadView') {
                // If specific lead is already selected
